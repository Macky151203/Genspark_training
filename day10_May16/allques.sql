...
do $$
	declare 
		cr cursor for select distinct s.name,s.email from enrollments e join students s on e.student_id=s.student_id where e.enrollment_id not in (select enrollment_id from certificates);
		rec record;
	begin 
		for rec in cr loop
...
begin 
	...
	values(p_student_id,p_course_id,current_date) returning enrollment_id into eid;
	if status then 
		insert into certificates (certificate_id,enrollment_id, issue_date, serial_no)
		values(5,eid,current_date,CONCAT('CERT-', p_course_id, '-',eid));
	end if;
	exception
	when others then
		rollback;
...
call sp_enroll_student2(2,2,true);
select * from enrollments;
select * from certificates;
